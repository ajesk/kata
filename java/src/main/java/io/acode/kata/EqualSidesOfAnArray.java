...
class EqualSidesOfAnArray {
    static int sumIndices(int start, int end, int[] arr) {
        return IntStream.range(start, end)
                .map(i -> arr[i])
                .sum();
    }

    static boolean compareIndices(int i, int[] arr) {
        return sumIndices(0, i, arr) == sumIndices(i + 1, arr.length, arr);
    }

    /**
     * This is not the best way to do this. I chose this method to play around with streaming methods rather than
     * keeping a running total.
     * @param arr
     * @return
     */
    public static int findEvenIndex(int[] arr) {
        int[] indices = IntStream.range(0, arr.length - 1)
                .filter(i -> compareIndices(i, arr))
                .toArray();
